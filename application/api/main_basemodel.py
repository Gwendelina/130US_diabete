...
# Création d'un formulaire d'entrée des variables
...
@app.post("/test_basemodel/")
def predict_basemodel(data: Inputs):
    transform_dict = data.dict()
    df = pd.DataFrame(transform_dict, columns = ['time_in_hospital', 'num_lab_procedures', 'num_procedures',
                                         'num_medications', 'number_outpatient', 'number_emergency',
                                           'number_inpatient', 'number_diagnoses', 'age'])
    
    model_joblib = joblib.load('model_joblib')    
    prediction = model_joblib.predict(df)[0]

    return {'Ce patient sera-t-il réhospitalisé?': 'Oui' if prediction == 1 else 'Non'}

# @app.post('/prediction')
# def prediction(data : Inputs):
#     initial_data = data.dict()
#     data = pd.DataFrame(initial_data, columns = ['time_in_hospital', 'num_lab_procedures', 'num_procedures',
#                                          'num_medications', 'number_outpatient', 'number_emergency',
#                                            'number_inpatient', 'number_diagnoses', 'age'])
